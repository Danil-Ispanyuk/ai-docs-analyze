...
type Matched = { document_id: string; name: string; content: string; page: number | null; similarity: number };
...
export async function POST(req: Request) {
  ...
  const { messages, documentIds }: { messages: ChatMessage[]; documentIds?: string[] } = await req.json();

  const lastMessage = messages[messages.length - 1];
  const question = lastMessage?.parts?.map((p) => (p.type === "text" ? p.text : "")).join("").trim() ?? "";
  ...
  for (const c of chunks) {
    ...
    if (!seen.has(key)) { seen.add(key); sources.push({ documentId: c.document_id, name: c.name, page: c.page }); }
  }

  const context = chunks.map((c, i) => `[#${i + 1} ${c.name}${c.page ? `, p.${c.page}` : ""}]\n${c.content}`).join("\n\n");
